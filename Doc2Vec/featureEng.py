...
class Vec():
    ...
    """
    def tf(self, df):
        tf_vectorizer = CountVectorizer(min_df=.05, max_df=.5, max_features=no_features, ngram_range=[1,2])
...
#            for j in range(len(list(chain.from_iterable(tag[i][i][i][:])))-1):
...
#        xvalid_tfidf =  tfidf_vect.transform(valid_x)
        ...
        # ngram level tf-idf 
        tfidf_vect_ngram = TfidfVectorizer(analyzer='word', token_pattern=r'\w{1,}', ngram_range=(4,4), max_features=5000)
        ...
        idf_df = pd.DataFrame(xtrain_tfidf_ngram.toarray(), columns=tfidf_vect.get_feature_names(), index=df.index)
        idf_df.shape
        pd.set_option('display.max_colwidth', -1)
        print("idf: ", idf_df)
...
#        xvalid_tfidf_ngram_chars =  tfidf_vect_ngram_chars.transform(valid_x)
        #return(xtrain_tfidf, xvalid_tfidf,xtrain_tfidf_ngram, xvalid_tfidf_ngram%time dtm_tf = tf_vectorizer.fit_transform(kiva_df['en_clean']), xtrain_tfidf_ngram_chars, xvalid_tfidf_ngram_chars)
if __name__ == "__main__":
    pd.set_option('display.max_colwidth', -1)
    vector = Vec()
    #df = vector.readCorp(r'C:\Users\kishite\Documents\Education\Queens\MMAI\MMAI891\Project\Ppython\DataOutPre\bgis_vendorPre_words.csv')  
...
#    matrix
#    export_csv = matrix.to_csv (r'C:\Users\kishite\Documents\Education\Queens\MMAI\MMAI891\Project\Ppython\DataOutMatrix\bgis_matrix_words.csv', index = None, header=True)
#    
